...
| 2026-08-06 | Phase 05 follow-up — purchase-order destination (continuous deploy from `main`) | Staging only — `eliza-crm.vercel.app` + Supabase `yhrdyyvayistqqwxawqr` | 1 applied (20260806150010) | Lint/typecheck/build clean; unit 117/117 | Constraint probe: an order with no destination is refused past draft, legacy row untouched; order creation restored for companies with no warehouse | Reverts and replaces the earlier service-only fix (commit ecf1841). Production still untouched and unauthorized |
