...
Implement finance foundations: chart of accounts (as approved at activation), expenses, supplier payments, financial corrections, and multi-currency handling.

## Business Outcome

Financial records are accurate, multi-currency-safe (numeric, base-currency values), approval-gated, and never silently edited.

## Included Scope

- Finance structures approved at activation (scope finalized with owner)
- Expenses and supplier payments with approvals
- Financial corrections as reversals/amendments only
- Multi-currency: amount, currency, exchange rate, base-currency value
- Invoice/PO financial linkage (Phases 04/07/09 records)

## Excluded Scope

- Full statutory accounting/ERP replacement unless owner approves scope at activation
- External accounting integrations (Phase 20)
- Everything belonging to other phases (see `MODULE_ROADMAP.md`)

## Dependencies

Phases 01, 04, 07, 09 complete for source records.

## Database Changes

Finance tables; numeric money everywhere; invoice-number and idempotency uniqueness.
