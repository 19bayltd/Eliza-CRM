...
Import and report Amazon performance including ACOS and TACOS with full traceability.

## Business Outcome

Amazon sales/advertising performance, ACOS, and TACOS are visible per account/marketplace/date range with freshness and traceability.

## Included Scope

- Amazon report imports (CSV first; SP-API via Phase 20 if approved), master §18 pipeline
- Sales + advertising metric storage with source identifiers
- ACOS/TACOS computed from stored source data (formulas in `GLOSSARY.md`)
- Amazon report views on the Phase 13 platform

## Excluded Scope

- Listing management or any Amazon write operations
- Everything belonging to other phases (see `MODULE_ROADMAP.md`)

## Dependencies

Phases 01, 13 complete.

## Database Changes

Amazon metric tables; idempotent imports; source IDs.
