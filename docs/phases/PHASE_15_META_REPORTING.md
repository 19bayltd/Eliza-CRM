...
Import and report Meta (Facebook/Instagram) advertising performance with full traceability.

## Business Outcome

Meta spend/performance is visible per company/account/date range with freshness status and import-job traceability.

## Included Scope

- Meta data import (CSV first; API via Phase 20 if approved) using master §18 pipeline
- Campaign/adset/ad-level metrics storage with source identifiers
- Meta report views on the Phase 13 platform

## Excluded Scope

- Ad management/write operations to Meta (never in scope without ADR)
...
Meta metric tables keyed by import job + source IDs; idempotent imports.
...
Meta report view/import permissions.
