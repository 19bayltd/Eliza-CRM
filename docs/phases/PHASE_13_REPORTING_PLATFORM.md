...
Build the shared reporting platform: report definitions, import-job framework hardening, freshness tracking, and traceability.

## Business Outcome

Every metric shows value, comparison, target, source, account, company, date range, last-updated, and freshness — nothing invented.

## Included Scope

- Report framework (definitions, parameters, caching strategy)
- Freshness/traceability metadata per `ELIZA_OS_MASTER_SPEC.md` §4.10
- Import-job framework generalized for external sources (master §18 pipeline)
- Department report views for existing modules

## Excluded Scope

- Meta/Amazon/SEO source integrations (Phases 15–17); executive dashboards (Phase 18)
- Everything belonging to other phases (see `MODULE_ROADMAP.md`)

## Dependencies

Phases 01–12 provide source data; minimum: Phase 01 + at least one data module.

## Database Changes

Report definition/import-job tables; idempotency keys.
