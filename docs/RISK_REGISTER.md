...
| R-017 | Stock has no value: quantities are tracked but not costed, so "what is this inventory worth" is unanswerable until Phase 12 | inventory, finance | High | Medium | Medium | Ledger entries carry the reference to their purchase receipt, so a costing layer can be added later without re-recording history | Engineering | Open | Phase 12 start |
| R-018 | A count run during heavy movement shows variances that are really timing: counting does not freeze stock, and the variance is measured against the snapshot taken when the count opened | inventory | Medium | Medium | Medium | Deliberate — freezing a warehouse is worse. Movements during the count remain in the ledger and are visible next to the correction; guidance is to count quiet periods | Engineering | Open | Each count |
